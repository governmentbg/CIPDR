$(function () {
    ...
    //#region За идентифициране на физическо лице
...
function setLoadPersonInfoButtonVisibility(pid) {
    ...
    $('.load-person-info').on("click", function () {
        showLoader('body');
        let actionUrl = '/Integration/GetPersonData';
...
function setLoadCompanyInfoButtonVisibility(cid) {
    ...
    $('.load-company-info').on("click", function () {
        showLoader('body');
        let actionUrl = '/Integration/GetCompanyData';
        let cid = $(this).closest('.cid');
        let cidType = cid.find('.ui.dropdown').dropdown('get value');
        get_async(actionUrl, {
            cidType: cidType,
    ...
    $('.autocomplete-with-category')
        .each(function () {
            $(this).search(
                {
                    apiSettings: {
                        url: `/Nomenclature/GetAutocompleteWithCategoryValues?query={query}&nomenclatureType=${$(this).data('nomenclatureType')}`,
                    },
                    minCharacters: 5,
                    type: 'category',
                    onSelect: function (result) {
