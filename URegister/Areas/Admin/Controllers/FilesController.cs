...
namespace URegister.Areas.Admin.Controllers
{
    ...
    public class FilesController : BaseController
    {
        ...
        [HttpPost]
        [Display(Name = "Подготвяне на PDF файл за визуален подпис")]
        public async Task<IActionResult> PreparePdfForVisualSignature([FromBody] PdfVisualSignatureData data)
        {
            string hash;
            string tempPdfId;
            if (data?.FileId == null)
            ...
            {
                Cordinates = new iText.Kernel.Geom.Rectangle(position.x, position.y, position.width, position.height),
                Location = "ИСЦИПР",
                PageNumber = 1,
                Reason = data.Reason,
        ...
        [HttpPost]
        public async Task<IActionResult> SignVisual(PreviewInfoViewModel model)
        {
            byte[] signedPdf;
