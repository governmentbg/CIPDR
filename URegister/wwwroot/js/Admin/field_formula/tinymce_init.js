...
function init_tinymce() {
    tinymce.init({        
        ...
        menubar: '',
        toolbar: "undo redo | mergetags", // Note: if a toolbar item requires a plugin, the item will not present in the toolbar if the plugin is not also loaded.
...
async function mergetag_list_func() {
    const templateParams = await get_async(`/Admin/FieldFormula/GetFormulaMemberCandidates?formParentId=${$('#FormParentId').val()}`, {});
    return templateParams.map((x) => {       
        return { title: x.text, value: x.value };       
    });
}
var tag_list;
$(async function () {
    ...
    $('#submit').submit(function () {
        $('#FormulaText').val(tinyMCE[0].getContent({ format: 'text' }));
    });
})
