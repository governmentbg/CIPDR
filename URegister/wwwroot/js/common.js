...
function initDynamicForms(addCallback, addDataFunc) {
    ...
    $('button.add-item').each(function (i, btn) {
        ...
        $(btn).click(async function () {
            ...
            await PerformAddItem(addItem, addData);
            if (addCallback) {
                addCallback();
            }
            return false;
...
async function downloadFile(url, request, inFileName) {
...
async function downloadPresignedFile(url, fileName) {
    try {
        const res = await fetch(
            url,
            {
